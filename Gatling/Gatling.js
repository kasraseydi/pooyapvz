...
export default class Gatling extends Sprite {
  constructor(...args) {
    super(...args);

    this.costumes = [
      new Costume("PeaShooter_Head", "./Gatling/costumes/PeaShooter_Head.svg", {
        x: 52,
        y: 46.333325,
      }),
      new Costume("blink 0", "./Gatling/costumes/blink 0.svg", {
        x: 1.9990330052961554,
        y: 21.50095439502445,
      }),
      new Costume("blink 1", "./Gatling/costumes/blink 1.svg", {
        x: 1.9901960784312962,
        y: 21.51633986928107,
      }),
      new Costume("blink 2", "./Gatling/costumes/blink 2.svg", {
        x: 1.9990349999999921,
        y: 21.500955000000005,
      }),
      new Costume("blink 3", "./Gatling/costumes/blink 3.svg", { x: 0, y: 0 }),
      new Costume(
        "PeaShooter_stalk_bottom",
        "./Gatling/costumes/PeaShooter_stalk_bottom.svg",
        { x: 20.030665204678343, y: 34.90902777777757 }
      ),
      new Costume(
        "PeaShooter_frontleaf",
        "./Gatling/costumes/PeaShooter_frontleaf.svg",
        { x: 40.4967320261438, y: 20 }
      ),
      new Costume(
        "PeaShooter_backleaf",
        "./Gatling/costumes/PeaShooter_backleaf.svg",
        { x: 22.388888888888914, y: 11 }
      ),
      new Costume(
        "GatlingPea_helmet",
        "./Gatling/costumes/GatlingPea_helmet.svg",
        { x: 52, y: 46.333335000000005 }
      ),
    ];

    this.sounds = [
      new Sound("plant", "./Gatling/sounds/plant.mp3"),
      new Sound("chompsoft", "./Gatling/sounds/chompsoft.mp3"),
      new Sound("gulp", "./Gatling/sounds/gulp.mp3"),
      new Sound("throw", "./Gatling/sounds/throw.mp3"),
      new Sound("throw2", "./Gatling/sounds/throw2.mp3"),
    ];
    ...
    this.vars.secondFirst = 0;
    this.vars.thirdFirst = 0;
  }
  ...
  *gridPos() {
    ...
    for (let i = 0; i < this.stage.vars.gridSpaceX.length; i++) {
      if (
        ...
        this.stage.vars.gatlingGrid = this.vars.grid;
        return;
      }
      this.vars.i++;
    }
    ...
    while (true) {
      ...
      if (this.toString(this.vars.state) === "grabbed") {
        ...
        if (this.toString(this.vars.first) === "yes") {
          yield* this.layer();
          this.stage.vars.grid.splice(this.vars.grid - 1, 1, "gatling");
          this.stage.vars.sun -= 250;
          this.vars.firstFirst = "yes";
          this.vars.secondFirst = "yes";
          this.vars.thirdFirst = "yes";
        }
        this.vars.first = "no";
        ...
        if (this.compare(this.vars.timer, 45) > 0) {
          if (
            ...
            if (this.toString(this.vars.firstFirst) === "yes") {
              this.stage.vars.peaQueueX.push(this.vars.x);
        ...
        if (this.compare(this.vars.timer, 50) > 0) {
          if (
            this.toString(
              this.itemOf(this.stage.vars.zombieY, this.vars.layer - 1)
            ) === "yes"
          ) {
            if (this.toString(this.vars.secondFirst) === "yes") {
              this.stage.vars.peaQueueX.push(this.vars.x);
              this.stage.vars.peaQueueY.push(this.vars.y);
              this.stage.vars.peaQueueDrawY.push(this.vars.y);
              yield* this.startSound(this.random(4, 5));
            ...
            this.vars.secondFirst = "no";
          }
        }
        if (this.compare(this.vars.timer, 55) > 0) {
          if (
            this.toString(
              this.itemOf(this.stage.vars.zombieY, this.vars.layer - 1)
            ) === "yes"
          ) {
            if (this.toString(this.vars.thirdFirst) === "yes") {
              this.stage.vars.peaQueueX.push(this.vars.x);
              this.stage.vars.peaQueueY.push(this.vars.y);
              this.stage.vars.peaQueueDrawY.push(this.vars.y);
              yield* this.startSound(this.random(4, 5));
  ...
  *startAsClone2() {
    ...
    if (
      ...
      this.broadcast("replace repea");
      this.audioEffects.volume = this.toNumber(this.stage.vars.volume);
      yield* this.startSound("plant");
      this.vars.state = "set";
    } else {
      this.stage.vars.timerReset = "gatling";
