...
export default class WinterMelon extends Sprite {
  constructor(...args) {
    super(...args);

    this.costumes = [
      new Costume("pos", "./WinterMelon/costumes/pos.svg", {
        x: 135,
        y: 94.50000000000004,
      }),
      new Costume(
        "Melonpult_melon",
        "./WinterMelon/costumes/Melonpult_melon.svg",
        { x: 27.5, y: 24 }
      ),
      new Costume("splash", "./WinterMelon/costumes/splash.svg", {
        x: 102.5,
        y: 102.5,
      }),
    ];
    ...
    this.sounds = [
      new Sound("melonimpact", "./WinterMelon/sounds/melonimpact.mp3"),
      new Sound("melonimpact2", "./WinterMelon/sounds/melonimpact2.mp3"),
    ];
  ...
  *whenIReceiveStart() {
    this.stage.vars.winterMelonQueueX = [];
    this.stage.vars.winterMelonQueueY = [];
    while (true) {
  ...
  *makeAttacks() {
    for (let i = 0; i < this.stage.vars.winterMelonQueueX.length; i++) {
      this.vars.x = this.itemOf(this.stage.vars.winterMelonQueueX, 0);
      this.vars.y = this.itemOf(this.stage.vars.winterMelonQueueY, 0);
      this.createClone();
      this.stage.vars.winterMelonQueueX.splice(0, 1);
      this.stage.vars.winterMelonQueueY.splice(0, 1);
    }
  }

  *melon() {
    for (let i = 0; i < 10; i++) {
      this.stage.vars.particleQueueType.push("winter melon");
