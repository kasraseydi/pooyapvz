...
export default class Sun2 extends Sprite {
  constructor(...args) {
    super(...args);
    ...
    this.costumes = [
      new Costume(
        ...
        "./Sun2/costumes/SunFlower_double_petals.svg",
        { x: 38, y: 29.5 }
      ),
      new Costume("glow", "./Sun2/costumes/glow.svg", {
        x: 49.583340000000106,
        y: 49.58334000000008,
      }),
      new Costume("SunFlower_head", "./Sun2/costumes/SunFlower_head.svg", {
        x: 28.5,
        y: 21.5,
      }),
      new Costume("blink 0", "./Sun2/costumes/blink 0.svg", {
        x: 12.504416282642097,
        y: 12.504032258064512,
      }),
      new Costume("blink 1", "./Sun2/costumes/blink 1.svg", {
        x: 12.499999999999943,
        y: 12.499999999999972,
      }),
      new Costume("blink 2", "./Sun2/costumes/blink 2.svg", {
        x: 12.504414999999995,
        y: 12.50403,
      }),
      new Costume("blink 3", "./Sun2/costumes/blink 3.svg", { x: 0, y: 0 }),
      new Costume(
        "PeaShooter_stalk_bottom",
        "./Sun2/costumes/PeaShooter_stalk_bottom.svg",
        { x: 20.030665204678343, y: 34.90902777777757 }
      ),
      new Costume(
        "PeaShooter_frontleaf",
        "./Sun2/costumes/PeaShooter_frontleaf.svg",
        { x: 40.4967320261438, y: 20 }
      ),
      new Costume(
        "PeaShooter_backleaf",
        "./Sun2/costumes/PeaShooter_backleaf.svg",
        { x: 22.388888888888914, y: 11 }
      ),
    ];

    this.sounds = [
      new Sound("plant", "./Sun2/sounds/plant.mp3"),
      new Sound("chompsoft", "./Sun2/sounds/chompsoft.mp3"),
      new Sound("gulp", "./Sun2/sounds/gulp.mp3"),
    ];
    ...
    this.vars.x = 0;
    this.vars.y = 0;
    this.vars.sin = 6575.249999999987;
    this.vars.ghost = 100;
    this.vars.grid = 0;
    this.vars.i = 46;
    this.vars.state = 0;
  ...
  *animate() {
    ...
    this.direction =
      Math.sin(this.degToRad(this.toNumber(this.vars.sin))) * 15 + 90;
    this.size = 50;
    this.goto(this.toNumber(this.vars.x) + 3, this.toNumber(this.vars.y) + 0);
    this.stamp();
    ...
    this.size =
      Math.sin(this.degToRad(this.toNumber(this.vars.sin) * 2)) * 2 + 65;
    this.direction =
    ...
    while (true) {
      ...
      if (this.toString(this.vars.state) === "grabbed") {
        ...
        if (this.toString(this.vars.first) === "yes") {
          this.stage.vars.grid.splice(this.vars.grid - 1, 1, "sun");
          ...
          if (
            ...
            if (
              ...
              if (this.toString(this.vars.flower) === "yes") {
                this.stage.vars.sunQueueX.push(this.vars.x);
                this.stage.vars.sunQueueY.push(this.vars.y);
              }
              this.vars.flower = "no";
            } else {
  ...
  *whenIReceiveReplaceSun() {
    if (this.compare(this.vars.grid, this.stage.vars.sunGrid) === 0) {
      this.deleteThisClone();
    }
  }
  ...
  *startAsClone2() {
    ...
    if (
      ...
      this.stage.vars.timerReset = "sun";
